...
export class JournalSearchApp extends HandlebarsApplicationMixin(ApplicationV2) {
    static DEFAULT_OPTIONS = {
        ...
        tag: "form",
        window: {
            title: "PHILS_UNIVERSAL_TRANSLATE.Search.Title",
    ...
    async _prepareContext(options) {
        ...
        const journalOptions = [
            { id: "all", name: game.i18n.localize("PHILS_UNIVERSAL_TRANSLATE.Search.AllJournals") },
    ...
    _performSearch() {
        ...
        try {
            ...
            ui.notifications.error(`Invalid RegEx: ${e.message}`);
            return;
        }
    ...
    static async onReplaceSingle(event, target) {
        ...
        ui.notifications.success(`Replaced on page "${page.name}"!`);
        this._performSearch();
    }
    ...
    static async onReplaceAll(event, target) {
        ...
        ui.notifications.success(`${totalReplacements} replacements completed across all journals!`);
        this._performSearch();
    }
