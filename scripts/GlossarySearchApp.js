...
export class GlossarySearchApp extends HandlebarsApplicationMixin(ApplicationV2) {
    static DEFAULT_OPTIONS = {
        ...
        tag: "form",
        window: {
            title: "PHILS_UNIVERSAL_TRANSLATE.Glossary.Title",
    ...
    static async onAddTerm(event, target) {
        ...
        const content = `
            <div style="padding: 10px; font-family: var(--font-primary);">
                <div style="margin-bottom: 12px;">
                    <label style="display:block; font-weight: bold; margin-bottom: 4px;">${game.i18n.localize("PHILS_UNIVERSAL_TRANSLATE.Glossary.ThOriginal")}:</label>
                    <input type="text" id="add-term-orig" style="width: 100%; padding: 6px; background: rgba(0,0,0,0.3); border: 1px solid #555; color: #fff; border-radius: 4px;" autofocus>
                </div>
                <div style="margin-bottom: 12px;">
                    <label style="display:block; font-weight: bold; margin-bottom: 4px;">${game.i18n.localize("PHILS_UNIVERSAL_TRANSLATE.Glossary.ThTranslation")}:</label>
                    <input type="text" id="add-term-trans" style="width: 100%; padding: 6px; background: rgba(0,0,0,0.3); border: 1px solid #555; color: #fff; border-radius: 4px;">
                </div>
                <div style="margin-bottom: 12px;">
                    <label style="display:block; font-weight: bold; margin-bottom: 4px;">${game.i18n.localize("PHILS_UNIVERSAL_TRANSLATE.Glossary.ThCategory")}:</label>
        ...
        new Dialog({
            title: game.i18n.localize("PHILS_UNIVERSAL_TRANSLATE.Glossary.BtnAddTerm"),
            content: content,
            buttons: {
                save: {
                    icon: '<i class="fas fa-plus"></i>',
                    label: game.i18n.localize("PHILS_UNIVERSAL_TRANSLATE.Glossary.BtnAddTerm"),
                    callback: async (html) => {
                        ...
                        if (orig && trans) {
                            ...
                            ui.notifications.success(game.i18n.format("PHILS_UNIVERSAL_TRANSLATE.Notifications.TermAdded", { term: orig }));
                            this.render(false);
                        }
                    }
                },
    ...
    static async onEditTerm(event, target) {
        ...
        new Dialog({
            title: `${game.i18n.localize("PHILS_UNIVERSAL_TRANSLATE.Glossary.Edit")}: ${orig}`,
            content: content,
            buttons: {
                save: {
                    icon: '<i class="fas fa-check"></i>',
                    label: game.i18n.localize("PHILS_UNIVERSAL_TRANSLATE.UI.BtnApplyFinalUpdate"),
                    callback: async (html) => {
                        ...
                        if (newOrig && newTrans) {
                            ...
                            ui.notifications.success(game.i18n.format("PHILS_UNIVERSAL_TRANSLATE.Notifications.TermUpdated", { term: newOrig }));
                            this.render(false);
                        }
                    }
                },
    ...
    static async onDeleteTerm(event, target) {
        ...
        Dialog.confirm({
            title: `${game.i18n.localize("PHILS_UNIVERSAL_TRANSLATE.Glossary.Delete")}: ${orig}`,
            content: `<p>${orig}?</p>`,
            yes: async () => {
                await UniversalGlossary.deleteGlossaryTerm(pageId, orig);
                ui.notifications.info(game.i18n.format("PHILS_UNIVERSAL_TRANSLATE.Notifications.TermDeleted", { term: orig }));
                this.render(false);
            }
        });
    }
    ...
    static onOpenGlossaryJournal(event, target) {
        ...
        if (journal) {
            ...
            ui.notifications.warn(game.i18n.localize("PHILS_UNIVERSAL_TRANSLATE.Notifications.NoGlossaryFound"));
        }
    }
}
