...
export class CampaignAuditorApp extends HandlebarsApplicationMixin(ApplicationV2) {
    static DEFAULT_OPTIONS = {
        ...
        tag: "div",
        window: {
            title: "PHILS_UNIVERSAL_TRANSLATE.Auditor.Title",
    ...
    async _prepareContext(_options) {
        ...
        let healthLabel = game.i18n.localize("PHILS_UNIVERSAL_TRANSLATE.Auditor.HealthGreat");

        if (audit.overallHealth < 60 || audit.errorCount > 0) {
            healthColor = "red";
            healthIcon = "fa-triangle-exclamation";
            healthLabel = game.i18n.localize("PHILS_UNIVERSAL_TRANSLATE.Auditor.HealthCritical");
        } else if (audit.overallHealth < 85 || audit.warningCount > 0) {
            healthColor = "yellow";
            healthIcon = "fa-circle-exclamation";
            healthLabel = game.i18n.localize("PHILS_UNIVERSAL_TRANSLATE.Auditor.HealthWarning");
        }
        ...
        let filteredJournals = (audit.journalAudits || []).map(j => {
            ...
            if (this.activeFilter === 'error') {
                ...
                pages = pages.filter(p => p.issues && p.issues.some(i => i.includes('Umlaute') || i.includes('Encoding')));
            }
            return {
                ...j,
                pageAudits: pages,
    ...
    static async onRefreshAudit(event, target) {
        this.auditData = await auditCampaignIntegrity();
        ui.notifications.info(game.i18n.localize("PHILS_UNIVERSAL_TRANSLATE.Notifications.AuditorRefreshed"));
        this.render(false);
    }

    static async onRepairBackups(event, target) {
        await repairAllBackupConflicts();
        ui.notifications.success(game.i18n.localize("PHILS_UNIVERSAL_TRANSLATE.Notifications.BackupsRepaired"));
        this.auditData = await auditCampaignIntegrity();
        this.render(false);
    }
    ...
    static async onRepairLinks(event, target) {
        ...
        ui.notifications.success(game.i18n.format("PHILS_UNIVERSAL_TRANSLATE.Notifications.LinksRepaired", { count: res.totalLinksRemapped }));
        this.auditData = await auditCampaignIntegrity();
        this.render(false);
    }
    ...
    static async onRepairMojibake(event, target) {
        ...
        ui.notifications.success(game.i18n.localize("PHILS_UNIVERSAL_TRANSLATE.Notifications.MojibakeRepaired"));
        this.auditData = await auditCampaignIntegrity();
        this.render(false);
    }
    ...
    static async onRestoreJournal(event, target) {
        ...
        if (journal) {
            Dialog.confirm({
                title: `${game.i18n.localize("PHILS_UNIVERSAL_TRANSLATE.Auditor.Restore")}: ${journal.name}`,
                content: `<p>${game.i18n.localize("PHILS_UNIVERSAL_TRANSLATE.Auditor.RestoreHint")} <strong>"${journal.name}"</strong>?</p>`,
                yes: async () => {
